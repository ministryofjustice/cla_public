...
# BrowserstackLocal runs as a persistent daemon process, and takes a few
# seconds to get going, so we run it as a non-blocking stream reader process,
# and wait until BrowserStack is ready before proceeding
bsl_args = "%s/%s -force %s localhost,8001,0" % (
    bin_path, BROWSERSTACK_BIN_NAME, os.environ.get('BS_KEY'))
p = subprocess.Popen(
    bsl_args.split(" "), stdin=subprocess.PIPE, stdout=subprocess.PIPE,
    stderr=subprocess.PIPE, shell=False)
bsl_process = NBSR(p.stdout)

while True:
    out = bsl_process.readline(0.1)
    if out:
        print out
        if 'You can now access' in out:
            break
