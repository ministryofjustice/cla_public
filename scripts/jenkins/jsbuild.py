...
backend_process = run_bg(
    "cd %s && %s/python manage.py testserver initial_category.json initial_oauth_clients.json --addrport 8000 --noinput --settings=cla_backend.settings.jenkins" % (backend_workspace.replace(' ', '\ '),backend_bin_path, ))
