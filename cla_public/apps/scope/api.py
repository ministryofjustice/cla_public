...
from cla_public.apps.checker.constants import CATEGORY_ID_MAPPING, F2F_CATEGORIES
...
class DiagnosisApiClient(object):
    ...
    def request_path(self, path=""):
        ...
        return "{host}{path}?{params}".format(
            host=current_app.config["BACKEND_API"]["url"], path=path, params=urllib.urlencode(request.args)
        )
    ...
    def move(self, choices_list=[]):
        ...
        steps, direction = self.get_steps_and_direction(previous_choices, choices_list)
    ...
    def save_category(self, category):
        ...
        if category not in F2F_CATEGORIES:
            post_to_eligibility_check_api(payload={"category": category})
    ...
    def save(self, response_json):
        ...
        steps = "\n\n".join(["%s: %s" % (i["question"], i["answer"]) for i in answers])
        steps += "\n\nOutcome: %s" % state

        session.checker.add_note(u"User selected", steps)

        if state == DIAGNOSIS_SCOPE.CONTACT and nodes:
            note = Markup(nodes[-1]["help_text"]).striptags()
            session.checker.add_note(u"Public Diagnosis note", note)
