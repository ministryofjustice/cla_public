...
class TestReviewPage(unittest.TestCase):
    ...
    def assert_answer_shown(self, question):
        self.assertIsNotNone(self.find_question(question), '"{0}" is not shown'.format(question))

    def assert_answer_not_shown(self, question):
        self.assertIsNone(self.find_question(question), '"{0}" is shown'.format(question))
    ...
    def set_about_you_answers(self, **kwargs):
        ...
        answers.update({"is_completed": True})
        with self.client.session_transaction() as session:
    ...
    def set_benefits(self, passported=None, *benefits):
        ...
        with self.client.session_transaction() as session:
            session.checker["YourBenefitsForm"] = {"benefits": benefits, "is_completed": True}
